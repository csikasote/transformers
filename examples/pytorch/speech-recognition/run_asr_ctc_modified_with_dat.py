...
from Wav2Vec2ForCTC_With_DAT import Wav2Vec2ForCTC
...
class DataCollatorCTCWithPadding:
    ...
    def __call__(self, features: list[dict[str, Union[list[int], torch.Tensor]]]) -> dict[str, torch.Tensor]:
        ...
        domain_labels = torch.tensor(
            [feature["domain_labels"] for feature in features], dtype=torch.long
        )
        ...
        batch["labels"] = labels
        batch["domain_labels"] = domain_labels
...
def main():
    ...
    if training_args.do_train:
        ...
        # Domain Column from the CSV file
        if data_args.domain_column_name not in raw_datasets["train"].column_names:
            raise ValueError(
                f"--domain_column_name {data_args.domain_column_name} not found in dataset '{data_args.dataset_name}'. "
                "Make sure to set `--text_column_name` to the correct text column - one of "
                f"{', '.join(raw_datasets['train'].column_names)}."
            )
    ...
    text_column_name = data_args.text_column_name # Text column for transcriptions
    domain_column_name = data_args.domain_column_name # Domain column names for DAT
    ...
    def prepare_dataset(batch):
        ...
        # Assumes your raw CSV has a column named "domain" with integer values, e.g. 0 or 1
        batch["domain_labels"] = int(batch[domain_column_name])
        return batch
