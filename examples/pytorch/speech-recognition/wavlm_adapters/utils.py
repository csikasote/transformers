...
class LibriSpeechDataset(object):
    ...
    def __getitem__(self, idx):
        ...
        array, sampling_rate = torchaudio.load(data['audio']) #Audio column name
        ...
        text = data['sentence'] # text column 
        file = data['audio'] # audio column
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                for step, inputs in enumerate(dataloaders_dict[phase]):
                    ...
                    #domain_labels = inputs['domain_labels'].to(device)
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        #asr_loss = outputs.loss.mean(dim=-1)

                        # domain adversarial loss
                        # outputs.features: [B, D]
                        # domain_preds = model.domain_classifier(outputs.features)
                        # domain_loss  = F.cross_entropy(domain_preds, domain_labels)
                        # total = ASR loss minus λ * domain_loss
                        # flip sign via gradient reversal layer already in domain_classifier
                        # total_loss = asr_loss + (phase=='train') * domain_loss
                    ...
                    if phase == 'train':
                        ...
                        # domain_log = domain_loss.item()
                        del loss
                        
                        if report_wandb:
                            wandb.log({'train/loss':loss_log})
                    epoch_loss += loss_log * minibatch_size
                    # epoch_domain_loss += (domain_loss.item() if phase=='train' else 0.0) * minibatch_size
                ...
                # epoch_domain_loss = epoch_domain_loss / len(dataloaders_dict['train'].dataset)
                ...
                if phase=='train':
                    ...
                    # print(f"Epoch {epoch+1}/{num_epochs}  {phase:^5}  ASR-Loss: {epoch_loss:.4f} " f"Domain-Loss: {epoch_domain_loss:.4f}  WER: {epoch_wer:.4f}")
                    if report_wandb:
                    ...
                    print('Epoch {}/{} | {:^5} |  Loss: {:.4f} WER: {:.4f}'.format(epoch+1,\
                                                                 num_epochs, phase, epoch_loss, epoch_wer))
                    #print(f"Epoch {epoch+1}/{num_epochs}  {phase:^5}  Loss: {epoch_loss:.4f} WER: {epoch_wer:.4f}")
                    #if report_wandb:
                    #    wandb.log({'val/epoch':epoch+1,,'val/epoch_loss':epoch_loss,'val/epoch_WER':epoch_wer})
